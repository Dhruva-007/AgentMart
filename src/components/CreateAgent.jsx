    ...
    <GlassCard hover glowColor="purple" delay={0.1}>
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-dark-100">Create AI Agent</h3>
            <p className="text-xs text-dark-400">Build and list your agent on the marketplace</p>
          </div>
        </div>
        <Button variant="glow" size="sm" onClick={() => setIsCreating(!isCreating)} icon={isCreating ? <X size={14} /> : <Plus size={14} />}>
          ...
          >
            {success ? (
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                ...
                <p className="text-xs text-dark-400">Your agent is now available in the marketplace</p>
              </motion.div>
            ) : (
              <div className="space-y-4 pt-2">
                {/* Agent Name */}
                <FormField label="Agent Name" icon={<Bot size={14} />} error={errors.name}>
                  <input
                    type="text"
                    value={form.name}
                    onChange={e => setField('name', e.target.value)}
                    placeholder="e.g., DeFi Risk Analyst"
                    maxLength={50}
                    className="w-full bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-2.5 text-sm text-dark-100 placeholder-dark-500 focus:outline-none focus:border-purple-500/30 transition-all duration-200"
                  />
                </FormField>

                {/* Description */}
                <FormField label="Description" icon={<FileText size={14} />} error={errors.description}>
                  <textarea
                    ...
                    rows={2}
                    maxLength={200}
                    className="w-full bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-2.5 text-sm text-dark-100 placeholder-dark-500 focus:outline-none focus:border-purple-500/30 transition-all duration-200 resize-none"
                  />
                </FormField>

                {/* Behavior / System Prompt */}
                <FormField label="Agent Behavior (System Prompt)" icon={<Sparkles size={14} />} error={errors.systemPrompt}
                  hint="Define how this agent thinks and responds">
                  <textarea
                    ...
                    rows={4}
                    maxLength={800}
                    className="w-full bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-2.5 text-sm text-dark-100 placeholder-dark-500 focus:outline-none focus:border-purple-500/30 transition-all duration-200 resize-none leading-relaxed"
                  />
                </FormField>

                {/* Output Style */}
                <FormField label="Output Style" icon={<Tag size={14} />}>
                  <div className="space-y-2">
                    <div className="grid grid-cols-1 gap-1.5">
                      {OUTPUT_STYLES.map((style, i) => (
                        <button key={i} onClick={() => { setField('outputFormat', style.value); setCustomOutput(false) }}
                          className={`text-left px-3 py-2 rounded-lg border text-xs transition-all duration-150 ${form.outputFormat === style.value && !customOutput ? 'bg-purple-500/10 border-purple-500/25 text-purple-300' : 'bg-white/[0.02] border-white/[0.05] text-dark-300 hover:bg-white/[0.05]'}`}>
                          {style.label}
                        </button>
                      ))}
                      <button onClick={() => setCustomOutput(true)}
                        className={`text-left px-3 py-2 rounded-lg border text-xs transition-all duration-150 ${customOutput ? 'bg-purple-500/10 border-purple-500/25 text-purple-300' : 'bg-white/[0.02] border-white/[0.05] text-dark-300 hover:bg-white/[0.05]'}`}>
                        Custom format...
                      </button>
                    </div>
                    {customOutput && (
                      <textarea value={form.outputFormat} onChange={e => setField('outputFormat', e.target.value)}
                        ...
                        rows={2} className="w-full bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-2.5 text-sm text-dark-100 placeholder-dark-500 focus:outline-none focus:border-purple-500/30 transition-all duration-200 resize-none" />
                    )}
                  </div>
                </FormField>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {CATEGORIES.map(cat => (
                        <button key={cat} onClick={() => setField('category', cat)}
                          className={`text-[10px] px-2.5 py-1 rounded-lg border font-medium transition-all duration-150 ${form.category === cat ? 'bg-purple-500/15 border-purple-500/25 text-purple-300' : 'bg-white/[0.02] border-white/[0.05] text-dark-400 hover:bg-white/[0.05]'}`}>
                          {cat}
                        </button>
                      ))}
                    </div>
                  ...
                  <FormField label="Card Color" icon={<Sparkles size={14} />}>
                    <div className="flex gap-2 flex-wrap">
                      {COLORS.map(color => (
                        <button key={color} onClick={() => setField('color', color)}
                          className={`w-7 h-7 rounded-lg transition-all duration-150 bg-gradient-to-br ${COLOR_MAP[color]} border ${form.color === color ? 'scale-125 border-white/30' : 'border-transparent'}`} />
                      ))}
                    </div>
                  </FormField>
                </div>
                ...
                <div className="flex items-center justify-between p-3 rounded-xl bg-white/[0.02] border border-white/[0.04]">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-dark-400">Creator Wallet (auto)</span>
                  </div>
                  <span className={`text-xs font-mono font-medium ${activeAccount ? 'text-green-400' : 'text-red-400'}`}>
                    {activeAccount ? `${activeAccount.address.slice(0, 8)}...${activeAccount.address.slice(-4)}` : 'Wallet not connected'}
                  </span>
                </div>
                ...
                {/* Submit */}
                <div className="pt-2">
                  <Button variant="glow" size="md" onClick={handleCreate} icon={<Plus size={14} />} className="w-full">
                    Create & List Agent
                  </Button>
  ...
  return (
    <div>
      <div className="flex items-center gap-1.5 mb-1.5">
        ...
        {hint && <span className="text-[10px] text-dark-500">— {hint}</span>}
      </div>
      {children}

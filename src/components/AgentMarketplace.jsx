    ...
    <GlassCard hover glowColor="purple" delay={0.1}>
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-white/90">Agent Marketplace</h3>
            <p className="text-xs text-white/50">{agents.length} agents available · Select to execute tasks</p>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-3 mb-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-white/[0.04] border border-white/[0.07] rounded-xl pl-9 pr-4 py-2
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide">
          ...
          {CATEGORY_FILTERS.map(cat => (
            <button key={cat} onClick={() => setActiveCategory(cat)}
              className={`flex-shrink-0 text-[10px] font-medium px-3 py-1 rounded-full border
                transition-all duration-150
                ${activeCategory === cat
                  ? 'bg-purple-500/15 border-purple-500/30 text-purple-300'
                  : 'bg-white/[0.04] border-white/[0.07] text-white/50 hover:bg-white/[0.07] hover:text-white/70'}`}>
              {cat}
            </button>
          ))}
        </div>
      ...
      <AnimatePresence>
        {selectedAgent && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -6 }}
            className="mb-4 flex items-center gap-3 p-3 rounded-xl bg-green-500/[0.08] border border-green-500/25"
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-green-400/75 truncate">
                Tasks will use this agent's configuration
              </p>
            </div>
          </motion.div>
        )}
      ...
      {filteredAgents.length === 0 ? (
        <div className="py-10 text-center">
          <Bot size={28} className="text-white/20 mx-auto mb-2" />
          <p className="text-sm text-white/50">No agents found</p>
          <p className="text-xs text-white/30 mt-1">Try adjusting your search or filter</p>
        </div>
      ) : (
  ...
  return (
    <motion.div
      ...
      className={`
        ...
        ${gradientClass}
        ${isSelected
          ? 'ring-2 ring-green-500/50 border-green-500/40'
          : ''}
      `}
    >
      {isSelected && (
        <motion.div
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
          className="absolute top-2 right-2 w-5 h-5 rounded-full bg-green-500/25 border border-green-500/40 flex items-center justify-center"
        >
      ...
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-white/50">{agent.category}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          {agent.rating > 0 && (
            <div className="flex items-center gap-1">
              ...
              <span className="text-[10px] font-medium text-white/60">{agent.rating}</span>
            </div>
          )}
          <div className="flex items-center gap-1">
            <Zap size={10} className="text-white/35" />
            <span className="text-[10px] text-white/50">{agent.usageCount} runs</span>
          </div>
        </div>
        ...
        <motion.button
          ...
          className={`
            text-[10px] font-semibold px-2.5 py-1 rounded-lg flex items-center gap-1 transition-all duration-150
            ${isSelected
              ? 'bg-green-500/15 text-green-400 border border-green-500/30'
              : 'bg-white/[0.06] text-white/60 border border-white/[0.09] hover:bg-purple-500/12 hover:text-purple-300 hover:border-purple-500/25'}`}
        >
      ...
      <div className="mt-2.5 pt-2.5 border-t border-white/[0.06] flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[9px] text-white/40">by {agent.creatorLabel}</span>
        </div>
        {!agent.isDefault && (
          <span className="text-[9px] text-purple-300/70 bg-purple-500/8 px-1.5 py-0.5 rounded">
            Your agent
          </span>
        )}
      </div>
